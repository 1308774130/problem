...
export const weightAVG = itemMap => {
    ...
    function* testing(referenceObj = [],errorItem, text = "", createStorage) {
        let flag = referenceObj.some(v => {
            ...
            if(totalItemWMap[v].errorChild){
                breakpoint = errorChild[errorItem]
                return BreakPoint()
            }
     ...
     * 二级检测
     */
    let secondTest = testing(firstKeyArr,'second', '二级维度', [])
    ...
    */
    if (!beakError) {
        let thirdTest = testing(secondKeyArr,'third', '三级维度', [])
    ...
    */
    if (!beakError) {
        let itemTest = testing(thirdKeyArr,'item', '评分项')
        beakError = itemTest.next().value
    }
